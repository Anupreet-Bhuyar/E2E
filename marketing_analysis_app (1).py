...
def generate_variants(input_text: str, components: dict, variant_count: int = 5) -> list:
    """Generate multiple copy variants using different angles and tones."""
    
    # Extract key elements from input
    has_urgency = "COMP19" in [k for k, v in components.items() if v["present"]]
    has_proof = "COMP2" in [k for k, v in components.items() if v["present"]]
    has_emotion = "COMP7" in [k for k, v in components.items() if v["present"]]
    ...
    variants.append({
        ...
        "angle": "Lead with transformation",
        "tone": "Inspiring",
        "copy": f"Unlock {input_text.split()[0:3]} and achieve your goals faster than ever before.",
        "components_used": ["COMP1", "COMP3", "COMP7"]
    })
    
    # Variant 2: Problem-pain
    variants.append({
        "name": "Problem-Aware",
        "angle": "Acknowledge the struggle",
        "tone": "Empathetic",
        "copy": f"Tired of the usual approach? {input_text.split()[0:5] if len(input_text.split()) >= 5 else input_text} reveals a better way.",
        "components_used": ["COMP6", "COMP21", "COMP12"]
    })
    ...
    if has_proof:
        variants.append({
            ...
            "angle": "Build trust through proof",
            "tone": "Authoritative",
            "copy": f"Join thousands who've transformed their results. {input_text.split()[0:4]} is now the proven standard.",
            "components_used": ["COMP2", "COMP16", "COMP23"]
        })
    else:
        variants.append({
            "name": "Credibility-Led",
            "angle": "Build trust through proof",
            "tone": "Authoritative",
            "copy": f"Backed by research and real results. {input_text.split()[0:4]} delivers measurable outcomes.",
            "components_used": ["COMP2", "COMP16", "COMP23"]
        })
    
    # Variant 4: Urgency-driven
    if has_urgency:
        variants.append({
            "name": "Urgency-Driven",
            "angle": "Create scarcity and momentum",
            "tone": "Compelling",
            "copy": f"The window is closing. Limited spots for {input_text.split()[0:2]}. Secure yours today.",
            "components_used": ["COMP19", "COMP26", "COMP9"]
        })
    else:
        variants.append({
            "name": "Urgency-Driven",
            "angle": "Create scarcity and momentum",
            "tone": "Compelling",
            "copy": f"Don't wait. Get {input_text.split()[0:3]} today and see results immediately.",
            "components_used": ["COMP19", "COMP26", "COMP3"]
        })
    
    # Variant 5: Curiosity hook
    variants.append({
        "name": "Curiosity Hook",
        "angle": "Stop the scroll",
        "tone": "Intriguing",
        "copy": f"Most people don't know this about {input_text.split()[0:2]}. Here's what changes everything.",
        "components_used": ["COMP20", "COMP14", "COMP11"]
    })
...
if st.button("🔍 Analyze Components", type="primary", use_container_width=True):
    if not input_text.strip():
        ...
        variants = generate_variants(input_text, components)
...
if st.session_state.analysis_results:
    ...
    # Copy Variants
    st.divider()
    st.subheader("✨ Generated Copy Variants")
    st.write("Different angles and tones to test:")
    
    for i, variant in enumerate(results["variants"], 1):
        with st.container(border=True):
            col1, col2 = st.columns([2, 1])
            with col1:
                st.write(f"**Variant {i}: {variant['name']}**")
                st.write(f"*Angle:* {variant['angle']}")
                st.write(f"*Tone:* {variant['tone']}")
            with col2:
                st.write(f"**Components:**")
                for comp in variant["components_used"]:
                    st.caption(comp)
            
            st.write(f"> {variant['copy']}")
            ...
            with col3:
                st.button("🔄 Regenerate", key=f"regen_{i}", use_container_width=True)
